...
func TeacherChangePasswordHandle(context *gin.Context) {
	...
	passwordQuery, err := model.NewTeacherAccountFlow().QueryTeacherPasswordByUserID(UserID) // 根据用户名获取数据库中密码
	if err != nil {
		log.Println(err)
		context.JSON(http.StatusInternalServerError, common.NewCommonResponse(1, "查询密码错误"))
		return
	}
	if passwordQuery != oldPassword { // 旧密码与数据库中密码验证失败
		context.JSON(http.StatusBadRequest, common.NewCommonResponse(1, "旧密码输入错误"))
		return
	}
	if err = model.NewTeacherAccountFlow().UpdateTeacherPasswordByUserID(UserID, newPassword); err != nil { // 更新数据库中密码
		log.Println(err)
		context.JSON(http.StatusInternalServerError, common.NewCommonResponse(1, err.Error())) // 更新失败
		return
	}
	// 更新成功
